      ...
      <div className="container mx-auto max-w-6xl relative z-10 w-full flex flex-col items-center">
        ...
        >
          <CarouselContent className="-ml-3 md:-ml-4">
            {specialties.map((item, index) => (
              <CarouselItem key={index} className="pl-3 md:pl-4 basis-[85%] sm:basis-1/2 lg:basis-1/3">
                <Card className="bg-foreground/[0.02] border-[var(--border)] h-full rounded-2xl overflow-hidden hover:bg-foreground/[0.04] transition-colors group">
                  <CardContent className="p-6 md:p-8 lg:p-10 flex flex-col h-full gap-5">
                    <div className="w-11 h-11 md:w-12 md:h-12 rounded-full bg-foreground/5 border border-[var(--border)] flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-500 text-electric-red">
                      <item.icon className="w-4 h-4 md:w-5 md:h-5" />
                    </div>
                    
                    <div className="flex flex-col gap-2">
                      <span className="font-mono text-[10px] uppercase text-foreground/40 tracking-widest">
                        {"// "} {item.topic}
                      </span>
                      <h4 className="text-foreground font-bold text-lg md:text-xl lg:text-2xl tracking-tight leading-snug">
                        {item.title}
                      </h4>
          ...
          <div className="hidden lg:block">
            <CarouselPrevious className="bg-transparent border-foreground/20 text-foreground hover:bg-foreground/10 hover:text-foreground left-[-3rem]" />
            <CarouselNext className="bg-transparent border-foreground/20 text-foreground hover:bg-foreground/10 hover:text-foreground right-[-3rem]" />
          </div>
        </Carousel>

        {/* Mobile drag hint */}
        <p className="mt-5 text-foreground/20 text-[10px] font-mono tracking-widest uppercase lg:hidden">
