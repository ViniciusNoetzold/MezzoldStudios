            ...
            <div className="grid grid-cols-3 gap-4 pt-6 border-t border-white/[0.07]">
              {stats.map((s, i) => (
                <div key={i} className="flex flex-col gap-1">
                  <span className="font-sans font-black text-2xl sm:text-3xl md:text-4xl text-white tracking-tight">
                    {s.value}
                  </span>
                  <span className="font-mono text-[10px] uppercase tracking-widest text-white/35 whitespace-pre-line leading-snug">
                    {s.label}
                  </span>
                </div>
              ))}
