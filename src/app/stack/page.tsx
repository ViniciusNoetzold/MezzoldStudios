...
export default function StackPage() {
  return (
    <>
      <Header />
      <main className="flex-1 pt-32 pb-24 px-6 relative z-10 min-h-[100dvh] bg-[#020202] overflow-hidden selection:bg-electric-red/30">
        ...
        <div className="container mx-auto max-w-5xl relative z-10 pt-10">
          <div className="mb-16 animate-in fade-in slide-in-from-bottom-8 duration-1000 ease-out fill-mode-both">
            <h1 className="font-sans font-black text-5xl md:text-7xl tracking-tighter mb-4 text-white uppercase drop-shadow-md">
              Stack<span className="text-electric-red animate-pulse">.</span>
            </h1>
            <p className="text-white/50 font-mono tracking-[0.2em] text-[10px] md:text-xs uppercase max-w-xl leading-relaxed">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {technologies.map((tech, i) => (
              ...
              >
                <GlassCard className="group relative text-center p-8 flex flex-col items-center justify-center space-y-4 h-52 border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/10 transition-all duration-500 overflow-hidden cursor-crosshair">
                  ...
                  <div className={`transition-all duration-500 ease-out transform group-hover:scale-110 group-hover:-translate-y-2 p-4 rounded-2xl bg-[#050505] border border-white/5 shadow-2xl ${tech.glow}`}>
                  ...
                  <div className="flex flex-col items-center transform transition-transform duration-500 group-hover:translate-y-1">
                    <h3 className="font-sans font-bold text-lg text-white mb-1 tracking-tight">{tech.name}</h3>
                    <span className="font-mono text-[9px] uppercase tracking-widest text-white/40">{tech.role}</span>
                  </div>
                </GlassCard>
              </div>
            ))}
